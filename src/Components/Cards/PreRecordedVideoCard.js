...
function PreRecordedVideoCard({
  ...
  const { DeletePreRecordedLecture, batchByCode } = useContext(BatchContext)
  ...
  const deletePreRecording = async () => {
    await DeletePreRecordedLecture(id, folderId)
  }
              ...
              {filename ? (
                filename.length > 18 ? (
                  <BootstrapTooltip title={filename} placement="top">
                    <p className={classes.cardTitle}>{`${filename.substring(
                      0,
                      18,
                    )}..`}</p>
                  </BootstrapTooltip>
                ) : (
                  <p className={classes.cardTitle}>{filename}</p>
                )
              ) : (
            ...
            {updatedAt && (
              <div>
                <p className={classes.cardDate}>
                  <span>{DisplayDate(updatedAt * 1000)}</span>
                </p>
              </div>
            )}
          </div>
...
export default PreRecordedVideoCard
